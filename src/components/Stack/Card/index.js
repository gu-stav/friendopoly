...
import { card, cardAsAnswer, question as questionStyle, footer, meta, metaContainer, answer as answerStyle, source as sourceStyle, questionText as questionTextStyle } from './style';
...
const Card = ({ question, index, length, continueStack, setStackCredit, ...rest }) => {
	...
	return (
		<div className={classnames(card, { [cardAsAnswer]: hasAnswered })} {...rest}>
			{!hasAnswered && (
				<p className={questionStyle}>
					...
					<span className={questionTextStyle}>{questionText}</span>
				</p>
			)}
			...
			<footer class={footer}>
				{hasAnswered === true ? (
					<Button type="button" theme="green" onClick={event => {
